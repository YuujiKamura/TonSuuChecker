...
export const TRUCK_SPECS: Record<string, TruckSpec> = {
  ...
  '増トン': {
    // 増トン車: 4tベースで積載量を増やした車両（5〜8t積載）
    // 見分け方: ホイールボルト8個、タイヤ大きめ、4tより大きく10tより小さい
...
6. **外部検索の活用（外形から機種を推定して検索）**:
   ...
   【増トン車の見分け方】
   - 増トン車 = 4tベースで積載量を増やした車両（5〜8t積載可能）
   - 見分け方: ホイールボルト8個（通常4tは6個）、タイヤが大きめ、後輪ダブルタイヤ
   - 4tより明らかに大きいが10tほどではない → 増トンの可能性
   - 検索時は「増トン ダンプ 荷台寸法」も有効
