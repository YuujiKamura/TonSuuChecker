...
`;
  ...
  return (
    <div className="max-w-4xl mx-auto p-2 sm:p-4 space-y-4 sm:space-y-8 pb-40">
      {/* 推定重量メイン表示 */}
      <div className="bg-slate-900 text-white p-4 sm:p-8 rounded-2xl sm:rounded-[3rem] shadow-2xl border border-slate-800 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-2 sm:mb-4">
            <p className="text-slate-400 text-xs sm:text-sm font-black uppercase tracking-wider sm:tracking-[0.2em]">AI 推定総重量</p>
            <div className="flex flex-col items-end gap-1 sm:gap-2">
              <span className="bg-slate-800 text-blue-400 text-[10px] sm:text-xs px-2 sm:px-4 py-1 rounded-full border border-slate-700 font-black">
                x{result.ensembleCount}
              </span>
              {errorRate !== null && (
                <span className={`text-[10px] sm:text-xs px-2 py-0.5 rounded-full font-black ${Math.abs(errorRate) < 5 ? 'bg-green-500' : 'bg-red-500'}`}>
          ...
          <div className="flex items-baseline gap-2 sm:gap-4">
            <span className="text-5xl sm:text-7xl md:text-8xl font-black text-yellow-500 tracking-tighter">
              {result.estimatedTonnage.toFixed(1)}
            </span>
            <span className="text-xl sm:text-2xl font-black text-slate-400 uppercase">t</span>
          </div>

          <div className="grid grid-cols-3 gap-2 sm:gap-4 mt-4 sm:mt-8 pt-4 sm:pt-8 border-t border-slate-800">
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-1 sm:gap-3">
              <div className="bg-slate-800 p-2 sm:p-3 rounded-xl">
                <Truck className="text-blue-400" size={20} />
              </div>
              <div className="text-center sm:text-left">
                <p className="text-slate-500 text-[8px] sm:text-[10px] uppercase font-black">車種</p>
                <p className="font-black text-xs sm:text-sm leading-tight">{result.truckType}</p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-1 sm:gap-3">
              <div className="bg-slate-800 p-2 sm:p-3 rounded-xl">
              ...
              <div className="text-center sm:text-left">
                <p className="text-slate-500 text-[8px] sm:text-[10px] uppercase font-black">最大積載</p>
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-1 sm:gap-3">
              ...
              <div className="text-center sm:text-left">
                <p className="text-slate-500 text-[8px] sm:text-[10px] uppercase font-black">体積</p>
                <p className="font-black text-xs sm:text-sm leading-tight">{result.estimatedVolumeM3.toFixed(1)}m³</p>
              </div>
            </div>
          </div>
          ...
          {result.maxCapacityReasoning && (
            <div className="mt-4 p-3 bg-slate-800/50 rounded-xl border border-slate-700">
              <p className="text-[10px] sm:text-xs text-purple-400 font-bold mb-1">推定根拠</p>
              <p className="text-xs sm:text-sm text-slate-400">{result.maxCapacityReasoning}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* 実測値入力セクション */}
      <div className="sticky top-2 sm:top-4 z-30 transition-all duration-500">
        <div className={`p-3 sm:p-6 rounded-2xl sm:rounded-[2rem] border-2 sm:border-4 transition-all shadow-xl ${isSaved ? 'bg-slate-900 border-green-500/50' : 'bg-blue-600 border-white'}`}>
          <div className="flex items-center justify-between mb-2 sm:mb-4">
            <h3 className={`text-xs sm:text-sm font-black flex items-center gap-2 uppercase ${isSaved ? 'text-green-400' : 'text-white'}`}>
              <Scale size={18} />
              {isSaved ? '保存済み' : '実測重量'}
            </h3>
            {isSaved && <CheckCircle2 className="text-green-500" size={20} />}
          </div>
          ...
          <div className="flex gap-2 sm:gap-4">
            <div className="relative flex-grow">
              <input
                ...
                placeholder="0.0"
                className={`w-full h-12 sm:h-16 px-4 sm:px-6 rounded-xl sm:rounded-2xl border-none outline-none font-black text-2xl sm:text-3xl transition-all ${
                  isSaved ? 'bg-slate-800 text-green-400' : 'bg-white text-slate-900'
                }`}
              />
              <span className={`absolute right-3 sm:right-6 top-1/2 -translate-y-1/2 font-black text-sm sm:text-lg ${isSaved ? 'text-green-700' : 'text-slate-300'}`}>
                t
              </span>
            </div>
            <button
              ...
              className={`w-12 sm:w-20 h-12 sm:h-16 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all active:scale-95 ${
                isSaved
                  ? "bg-green-500 text-white"
                  : "bg-slate-900 text-white hover:bg-black"
              }`}
            >
              {isSaved ? <Check size={24} strokeWidth={4} /> : <Save size={20} />}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-8">
        {/* 画像と車両情報 */}
        <div className="bg-white rounded-2xl sm:rounded-[2rem] overflow-hidden shadow-xl border border-slate-200">
          <div className="p-3 sm:p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
            <span className="font-black text-slate-700 flex items-center gap-2 text-sm sm:text-base uppercase">
              ...
              {isEditingPlate ? (
                <div className="flex items-center gap-2 bg-blue-50 p-1.5 rounded-lg border border-blue-200">
                  <input
                    ...
                    className="text-sm font-black border border-blue-300 rounded px-2 py-1.5 w-24 outline-none uppercase"
                    autoFocus
                  />
                  <button onClick={handlePlateSave} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-black">OK</button>
                </div>
              ) : (
                <button
                ...
                >
                  <div className="bg-slate-900 text-white px-3 py-1.5 rounded-lg flex items-center gap-2 group-hover:bg-black transition-colors">
                    <span className="text-sm font-black tracking-wider">{result.licenseNumber || '----'}</span>
          ...
          <div className="p-2">
            {imageUrls.map((url, i) => (
              <img key={i} src={url} className="w-full h-auto aspect-video object-cover rounded-xl" />
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          <div className="bg-white p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] shadow-xl border border-slate-200">
            <h3 className="text-sm sm:text-base font-black text-slate-800 mb-3 sm:mb-4 flex items-center gap-2 uppercase">
              <Info className="text-blue-500" size={18} />
              AI判断理由
            </h3>
            <div className="bg-slate-50 p-3 sm:p-4 rounded-xl border border-slate-100 mb-4 sm:mb-6">
              <p className="text-slate-600 text-sm sm:text-base leading-relaxed">"{result.reasoning}"</p>
            </div>

            <div className="w-full">
              <p className="text-[10px] sm:text-xs font-black text-slate-400 mb-3 sm:mb-4 uppercase">材質構成</p>
              <div className="space-y-3">
                {result.materialBreakdown.map((item, index) => (
                  <div key={index}>
                    <div className="flex justify-between text-xs sm:text-sm font-black text-slate-700 mb-1">
      ...
      {/* AIに質問セクション */}
      <div className="bg-slate-900 rounded-2xl sm:rounded-[2rem] shadow-xl border border-slate-800 overflow-hidden">
        <button
          onClick={() => setShowChat(!showChat)}
          className="w-full p-4 sm:p-6 flex items-center justify-between text-white hover:bg-slate-800 transition-colors"
        >
          <div className="flex items-center gap-3 sm:gap-4">
            ...
            <div className="text-left">
              <h3 className="text-sm sm:text-base font-black uppercase">AIに質問</h3>
              <p className="text-xs text-slate-400 hidden sm:block">なぜこの推定になったか詳しく聞く</p>
            </div>
          </div>
